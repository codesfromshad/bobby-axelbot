...
export interface RealTimeDataClientArgs {
   ...
   */
  onMessage?: (client: RealTimeDataClient, message: RTDSMessage) => void;
...
export class RealTimeDataClient {
  ...
  private readonly onConnect: ((client: RealTimeDataClient) => void) | undefined;

  /** Callback function executed when a custom message is received */
  private readonly onCustomMessage:
    | ((client: RealTimeDataClient, message: RTDSMessage) => void)
    | undefined;
  ...
  private ping = async () => {
    if (this.ws.readyState !== WebSocket.OPEN) {
      return console.warn(
        "Socket not open. Ready state is:",
        this.ws.readyState
      );
    }
  ...
  private onMessage = (event: MessageEvent): void => {
    if (typeof event.data === "string" && event.data.length > 0) {
      if (this.onCustomMessage && event.data.includes("payload")) {
        ...
        this.onCustomMessage(this, message as RTDSMessage);
      } else {
